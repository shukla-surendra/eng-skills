...
- **Glossary table at the end of any new chapter/doc** that uses non-trivial vocabulary or idioms: `Term/Phrase | Meaning`, one line each, mirroring `stories.md` and the tables in `Communication-Mastery/02_Thinking_Frameworks/06_prerequisite_stacking_and_the_elsewhere_effect.md`.
- **Index + anchors** at the top of any multi-section doc, with `[↑ Back to index](#index)` after each section — see `assertiveness-vocal-presence.md` or any `Communication-Mastery` chapter for the pattern.
